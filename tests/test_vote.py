...
from .conftest import authorized_client,test_posts,test_user
...
def test_user_can_vote_post_twise(authorized_client,test_vote,test_user,test_posts):
    res = authorized_client.post("/vote",json={'post_id':test_posts[0].id,'dir':1})
    assert res.status_code == 409


def test_user_can_vote_does_not_exists_post(authorized_client,test_user,test_posts):
