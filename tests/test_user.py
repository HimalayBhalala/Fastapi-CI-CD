...
def test_user_login(test_user,client):
    ...
    tokenData = schemas.Token(**res.json())
    token_decode = jwt.decode(tokenData.access_token,settings.secret_key,settings.algorithm)
    assert test_user['id'] == token_decode.get('user_id')
    assert tokenData.access_token == res.json().get('access_token')
    assert tokenData.token_type == "bearer"
    assert res.status_code == 200
